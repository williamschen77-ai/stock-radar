...
// ─── API calls (via Vercel serverless) ───────────────────────
async function fetchQuoteAndCandles(code, range = "6mo") {
  const res = await fetch(`/api/quote?symbol=${code}.TW&range=${range}&interval=1d`);
  if (!res.ok) throw new Error("API error");
  return res.json();
}

async function fetchInstitutional(code) {
  const res = await fetch(`/api/institutional?code=${code}`);
  if (!res.ok) throw new Error("API error");
...
async function fetchAIAnalysis(code, name, quote, instData) {
  const instSummary = instData.slice(-3).map(d =>
    `${d.date} 外資${d.foreign > 0 ? "+" : ""}${d.foreign} 投信${d.trust > 0 ? "+" : ""}${d.trust} 自營${d.dealer > 0 ? "+" : ""}${d.dealer}`
  ).join("；") || "資料不足";
  return aiJSON(`你是台股分析師。針對「${name}（${code}）」，現價${quote?.price}，近期三大法人：${instSummary}。
只回傳JSON不要其他文字：
{"summary":"2句話概述近期基本面與籌碼狀況","consensus":"1句話法人共識評估","risk":"1句話主要風險","news":[{"title":"標題","date":"2026-07-XX","sentiment":"正面"},{"title":"標題","date":"2026-07-XX","sentiment":"中性"},{"title":"標題","date":"2026-07-XX","sentiment":"負面"}]}`);
}

async function fetchAIPrediction(name, candles) {
  const recent = candles.slice(-10).map(c => `${c.date} C:${c.close}`).join(" | ");
  return aiJSON(`根據${name}近10日收盤（${recent}），預測未來5個交易日。只回傳JSON陣列不要其他文字：
[{"date":"2026-07-08","close":0,"high":0,"low":0},{"date":"2026-07-09","close":0,"high":0,"low":0},{"date":"2026-07-10","close":0,"high":0,"low":0},{"date":"2026-07-11","close":0,"high":0,"low":0},{"date":"2026-07-14","close":0,"high":0,"low":0}]`);
}
...
function calcMA(candles, period) {
  return candles.map((_, i) => {
    ...
    return +(s.reduce((a, c) => a + c.close, 0) / period).toFixed(2);
  });
}
...
function CandleChart({ candles, predicted = [], maLines = {}, showMA = {} }) {
  ...
  const maConfigs = [
    { key:"ma5",   color: T.ma.ma5   },
    { key:"ma20",  color: T.ma.ma20  },
    { key:"ma60",  color: T.ma.ma60  },
    { key:"ma120", color: T.ma.ma120 },
    { key:"ma240", color: T.ma.ma240 },
  ];

  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display:"block" }}>
      {Array.from({length:6}).map((_,i) => {
        const val = minP + (i/5)*(maxP-minP), y = toY(val);
        return <g key={i}>
      ...
      {maConfigs.map(({key,color}) => {
        ...
        return pts.length > 1
          ? <polyline key={key} points={pts.join(" ")} fill="none" stroke={color} strokeWidth="1.2" opacity="0.85" strokeLinejoin="round"/>
          : null;
      })}
      {candles.map((c,i) => {
        const x=toX(i), isUp=c.close>=c.open, col=isUp?T.buy:T.sell;
        const top=toY(Math.max(c.open,c.close)), bot=toY(Math.min(c.open,c.close));
        return <g key={i}>
      ...
      {predicted.length>0 && (() => {
        ...
        return <g>
          ...
          <text x={toX(si)+2} y={pad.t+10} fill={T.yellow} fontSize="9">▶ AI預測</text>
        </g>;
      })()}
...
function VolumeChart({ candles }) {
  const W=800, H=80, pad={l:62,r:16,t:8,b:20};
  const cW=W-pad.l-pad.r, cH=H-pad.t-pad.b;
  const n=candles.length, maxV=Math.max(...candles.map(c=>c.vol),1);
  const bw=Math.max(2,Math.floor(cW/n)-1);
  const toX=i=>pad.l+(i/n)*cW+bw/2;
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}>
...
function InstBarChart({ data }) {
  ...
  const W=800, H=160, pad={l:62,r:16,t:20,b:28};
  const cW=W-pad.l-pad.r, cH=H-pad.t-pad.b;
  const vals=data.flatMap(d=>[Math.abs(d.foreign),Math.abs(d.trust),Math.abs(d.dealer)]);
  const maxV=Math.max(...vals,1);
  const n=data.length, bw=Math.max(3,Math.floor(cW/n/4));
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}>
      ...
      {data.map((d,i)=>series.map((s,j)=>{
        const val=d[s.key], y=val>=0?toY(val):zero, h=Math.max(1,Math.abs(toY(val)-zero));
...
const TABS = [{id:"kline",label:"K線圖"},{id:"inst",label:"三大法人"},{id:"dates",label:"關鍵突破"},{id:"ai",label:"AI 分析"}];
const MA_OPTIONS = [
...
const RANGE_OPTIONS = [
  {label:"1個月",range:"1mo"},{label:"3個月",range:"3mo"},
  {label:"6個月",range:"6mo"},{label:"1年",range:"1y"},{label:"2年",range:"2y"},
];
...
export default function App() {
  ...
  // Fetch candles + quote
  useEffect(() => {
    setLoading(true); setError(""); setCandles([]); setQuote(null); setPredicted([]);
    fetchQuoteAndCandles(stockCode, range)
      .then(d => { setCandles(d.candles); setQuote(d.quote); })
      .catch(() => setError("無法載入資料，請確認股票代號是否正確"))
  ...
  useEffect(() => {
    ...
    fetchInstitutional(stockCode)
      .then(setInstData).catch(()=>setInstData([])).finally(()=>setInstLoading(false));
  }, [stockCode]);

  // AI analysis + prediction (fires after candles loaded)
  useEffect(() => {
  ...
  return (
    <div style={{background:T.bg,minHeight:"100vh",color:T.text,
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,
        padding:"14px 20px",display:"flex",alignItems:"center",gap:28,minHeight:68}}>
        {loading
          ? <div style={{color:T.muted}}>⏳ 載入中…</div>
          : error
          ? <div style={{color:T.sell,fontWeight:600}}>{error}</div>
          : quote ? <>
            <div>
      ...
      {/* Tabs */}
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 20px",display:"flex"}}>
        {TABS.map(t=>(
      ...
      <main style={{padding:"16px 20px"}}>
                  ...
                  {predicted.map((p,i)=>{
                    ...
                    return (
                      <div key={i} style={{background:T.surface,borderRadius:8,padding:"10px 12px",border:`1px solid ${T.border}`}}>
                        ...
                        <div style={{fontSize:10,color:pctColor(diff),marginTop:2}}>{diff>=0?"+":""}{diff}</div>
                      ...
                      <tr key={i} style={{borderBottom:`1px solid ${T.border}30`,
                        ...
                        {[d.foreign,d.trust,d.dealer,d.total].map((v,j)=>(
                          <td key={j} style={{padding:"9px 14px",textAlign:"right",
                            fontWeight:j===3?700:400,color:v>0?T.buy:v<0?T.sell:T.muted}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                  {[["外資",totF,T.accent],["投信",totT,T.yellow],["自營",totD,T.purple]].map(([l,v,c])=>(
                    <div key={l} style={{background:T.card,borderRadius:10,
                      ...
                      <div style={{fontSize:22,fontWeight:800,color:v>0?T.buy:v<0?T.sell:T.muted}}>
                        {v>0?"+":""}{v?.toLocaleString()}
                      </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            {keyDates.map((k,i)=>(
              <div key={i} style={{background:T.card,borderRadius:12,
                ...
                <div>
                  <div style={{fontWeight:800,fontSize:15,color:k.type==="突破"?T.buy:T.sell}}>{k.type}</div>
        ...
        {tab==="ai" && (
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {aiLoading&&<div style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,
                    ...
                    {aiData.news.map((n,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                        ...
                        <span style={{fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:20,
                          background:sentCol(n.sentiment)+"22",color:sentCol(n.sentiment)}}>{n.sentiment}</span>
                      </div>
                    ))}
                  </div>
                </div>
